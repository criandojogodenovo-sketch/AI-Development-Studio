...
test('V1 — withPoskliVersion define a versão dentro do contexto', async () => {
  ...
  await withPoskliVersion('expposkli-1.0', async () => {
    assert.equal(requestPoskliVersion(), 'expposkli-1.0')
    return null
  })
...
test('V2 — versão inválida/vazia/ausente: executa SEM override (env decide)', async () => {
  for (const invalid of ['9.9', 'expposkli-2.0', '   ', '', undefined]) {
    let seen: string | undefined = 'sentinela'
    await withPoskliVersion(invalid, async () => {
      seen = requestPoskliVersion()
      return null
    })
    assert.equal(seen, undefined, `versão ${JSON.stringify(invalid)} não deve criar override`)
  }
})

test('V3 — herança através de awaits aninhados (propagação orquestrador→agente)', async () => {
  await withPoskliVersion('expposkli-1.1', async () => {
    await delay(1)
    await (async () => {
      await delay(1)
      assert.equal(requestPoskliVersion(), 'expposkli-1.1', 'contexto deve sobreviver a awaits aninhados')
    })()
    return null
  })
})
...
test('V4 — valor de retorno da fn é preservado', async () => {
  const out = await withPoskliVersion('1.0-flash', async () => {
    assert.equal(requestPoskliVersion(), '1.0-flash')
    return 42
  })
  assert.equal(out, 42)
})

test('V5 — todas as 6 versões são aceitas como override válido', async () => {
  for (const v of ['0.1', '0.2', '0.3.1', '1.0-flash', 'expposkli-1.0', 'expposkli-1.1']) {
